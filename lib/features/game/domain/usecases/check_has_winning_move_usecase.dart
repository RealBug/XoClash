...
class CheckHasWinningMoveUseCase {
  CheckHasWinningMoveUseCase({CheckWinnerUseCase? checkWinnerUseCase})
  ...
  bool execute(List<List<Player>> board, Player player) {
    final testGameState = GameState(board: board, currentPlayer: player, status: GameStatus.playing);
    final result = _checkWinnerUseCase.execute(testGameState);
    return result.status == GameStatus.xWon && player == Player.x || result.status == GameStatus.oWon && player == Player.o;
  }
}
